...
### Description
Build a high-performance text generation API using Candle that supports multiple LLM models with caching, streaming, and production-grade error handling.

### Functional Requirements
1. Serve multiple LLM models (Llama-2, Mistral, CodeLlama) via REST API
2. Implement response caching to avoid regenerating identical prompts
3. Support temperature-based sampling and top-k/top-p filtering
4. Provide streaming responses for long generations
5. Include health checks and metrics endpoints
6. Support CPU and GPU inference with runtime device selection
7. Implement request batching for improved throughput
8. Include comprehensive logging and error tracking

### Main Components
- **Model Manager**: Handles loading/unloading models dynamically
- **Tokenizer Service**: Shared tokenizer instance with caching
- **Generation Engine**: Core inference loop with sampling strategies
- **Cache Layer**: LRU cache for prompt-response pairs
...
### Success Metrics
- >100 tokens/second generation speed on CPU for 7B models
- <500ms latency for first token (cold start)
- 95th percentile latency < 2s for 256-token generations
- Support 100+ concurrent requests with batching
- Memory usage < 8GB for 7B model in FP16
