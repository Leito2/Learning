...
| 43 | **Crear curso AI Compliance and Governance (06/25) — Tier-2 #3** | 6 notas, 3,345 líneas, English. Welcome (regulatory landscape 2026: EU AI Act, NIST AI RMF, ISO 42001, Colorado AI Act, NYC Local Law 144, China measures; por qué compliance engineering es table-stakes para roles enterprise) + EU AI Act 2024 Risk Classification (4 risk tiers: unacceptable [banned] / high [strict compliance] / limited [transparency] / minimal [voluntary]; 12 high-risk obligations per Articles 9-17,43,48,49,72; penalties up to €35M / 7% global turnover; explicit prohibited practices Article 5: social scoring, real-time biometric ID, emotion recognition; enforcement timeline Feb 2025 → Aug 2026 → Aug 2027; risk classifier tool + real cases: Clearview €30M, HireVue, ChatGPT ban) + Model Cards and Datasheets (Mitchell 2019 framework [9 sections: model details, intended use, factors, metrics, evaluation data, training data, quantitative analyses, ethical considerations, caveats]; Gebru 2018 framework [7 sections: motivation, composition, collection process, preprocessing, uses, distribution, maintenance]; HuggingFace cards template; mapping to EU AI Act Articles 11, 10, 13; automated generator from training metadata) + Bias and Fairness AIF360 (5 sources: historical, sampling, measurement, algorithmic, deployment; 5 canonical metrics: demographic parity, equal opportunity, predictive parity, calibration, counterfactual; **impossibility theorem**: cannot satisfy all simultaneously; IBM AIF360 toolkit with 11 mitigation algorithms; LLM-specific bias: stereotyping, exclusion, toxicity, cultural bias, BBQ benchmark, Detoxify; intersectional analysis) + Red-Teaming and Adversarial Testing (threat taxonomy: prompt injection direct/indirect, jailbreaking [DAN, roleplay, multi-step, encoding], bias exploits, data extraction, cross-attack chains; **Garak** from NVIDIA with 30+ probe categories; **PyRIT** from Microsoft with multi-turn attacks [Crescendo, Skeleton Key, PAIR, TAP]; severity classification [critical/high/medium/low]; production pipeline: Garak 10m → PyRIT 30m → Bias 15m → Custom 10m; CI integration blocks deploy on critical findings; mapping to Articles 9, 10, 15) + Capstone (compliance pipeline for credit-scoring LLM at bank: 4 artifacts combined into audit-ready PDF; risk_classifier.py + model_card_gen.py + datasheet_gen.py + bias_audit.py + red_team.py + report_gen.py; full CI/CD integration with critical-findings block deploy; 11th portfolio project: senior compliance engineer skill). Cierra el gap crítico: AI compliance engineering tenía solo 1 nota Spanish básica (12/30/04), vault carecía de cobertura completa para EU AI Act + model cards + bias + red-teaming. Main thread, 6 sequential Writes, 0 subagents. Cross-links: 06/15 LLM Security, 06/20 RAG Eval, 06/22 Instructor, 09/31 Phoenix, 09/34 OTel, 09/36 LangFuse, 12/30 Legal y Compliance en IA (Spanish primer), 10/22 Cloud Computing (BAA, SOC 2). **Variance note:** 3,345 líneas vs 2,800 target (+19%). Notas 01 (12 high-risk obligations + 8 real cases + risk classifier tool), 02 (model cards + datasheets + HuggingFace templates + automation), 03 (5 sources bias + 5 metrics + impossibility theorem + AIF360 + LLM-specific + BBQ benchmark + Detoxify), 04 (Garak + PyRIT 5 strategies + severity classification + production pipeline + CI), 05 (full capstone con 4 artifacts + PDF report generation + CI gate) sobre target por la profundidad compliance y el capstone end-to-end. | `06 - Large Language Models/25 - AI Compliance and Governance/` (6 files) |
| 44 | **Crear curso Real-time ML Systems (09/40) — Tier-2 #4** | 6 notas, 3,279 líneas, English. Welcome (shift from batch to real-time; 4 patterns: streaming features, online inference, CDC, drift detection; 12 portfolio project) + Streaming Feature Engineering (Kafka KRaft mode 2024 sin ZooKeeper; **Bytewax** replaces Faust (archived 2024); 4 aggregations: count/sum/mean/percentile; 3 window types: tumbling/sliding/session; Feast online store integration con <5ms lookup; late-arrival via watermarks; fault tolerance via checkpointing; fraud detection example completo) + Online Inference (model loaded una vez en memoria + pre-warming; request batching 32 reqs / 10ms para 5-10× throughput; async I/O paralelo para feature retrieval con asyncio.gather; connection pooling con httpx + HTTP/2 multiplexing; backpressure: bounded queue + 503 con Retry-After; graceful degradation con fallback model; LLM-specific: token streaming TTFT, request coalescing, prompt caching; fraud detection service example con <50ms p99) + Change Data Capture (Debezium para PostgreSQL con `pgoutput` logical decoding; CDC patterns: log-based/trigger-based/timestamp-based/snapshot+diff; Kafka Connect para streaming ETL; schema evolution via Schema Registry; CDC → Feast integration; failure modes: Debezium down, WAL overflow, schema breaks; S3 archive para disaster recovery) + Drift Detection (3 tipos: concept/data/label; 5 algorithms: ADWIN [default] / Page-Hinkley / EDDM / DDM / CUSUM; **River ML** framework; confirmation: 3+ consecutive detections antes de trigger; throttling: max 1 retrain/hora; LangFuse integration para unified observability; Evidently → Prometheus exporter; calibration con historical data; case study: LLM Eval Suite drift) + Capstone (fraud detection end-to-end pipeline: docker-compose.yml único boot con Postgres + Kafka + Debezium + Bytewax + Feast + Redis + FastAPI + Retrainer + Prometheus + Grafana; init.sql con publication `cdc_ml`; FraudDetectionModel con sklearn; Bytewax pipeline `cdc.public.transactions` → 5-min tumbling windows → Feast write; FastAPI inference con Feast features + asyncio.gather; ADWIN drift detector como background process; retrainer triggered on drift, pulls last 30 days from Postgres, trains new model, blue/green deploy; Prometheus + Grafana dashboards; 12th portfolio project: real-time ML skill). Cierra el gap crítico: real-time ML tenía solo notas dispersas (10/29/01 Kafka, 10/30 WebSockets, 09/27 Feast) pero faltaba el pipeline completo. Main thread, 6 sequential Writes, 0 subagents. Cross-links: 09/22 E2E ML Project, 09/27 Feast, 09/31 Evidently, 09/36 LangFuse, 09/39 Incident Response, 10/22 Cloud Computing, 10/29 Kafka, 13/06 Go ML Backend. **Variance note:** 3,279 líneas vs 2,800 target (+17%). Notas 02 (model pre-loading + request batching + backpressure + graceful degradation), 03 (CDC patterns + Debezium + Schema Registry + Feast integration), 04 (5 algorithms + confirmation + throttling + calibration + LLM eval case study), 05 (single docker-compose + 9-service architecture + drift-triggered retraining) sobre target por la profundidad operativa. | `09 - MLOps y Produccion/40 - Real-time ML Systems/` (6 files) |
| 45 | **Crear curso Multimodal Production RAG (06/26) — Tier-2 #5** | 6 notas, 3,287 líneas, English. Welcome (60% of enterprise data is non-text; 6 modalities: PDF/image/video/audio/table/chart; production stack: Unstructured + ColPali + CLIP + GPT-4o vision) + PDF Parsing and Extraction (PDF complexity matrix; PyMuPDF fast text 10-100× faster than pdfplumber; pdfplumber tables + layout; Unstructured.io `hi_res` strategy with Detectron2 layout detection; Marker clean markdown for equations/tables/figures; OCR with Tesseract + PaddleOCR for non-English; TableTransformer for image-based tables; complete legal document pipeline with scanned detection + OCR fallback; Airbnb listing example) + Visual Document Retrieval ColPali (visual doc RAG breakthrough; ColPali/ColQwen2 architecture: PDF → image → vision encoder → multi-vector embeddings → MaxSim late interaction; **30%+ accuracy improvement** on technical docs; ColQwen2 with Qwen2-VL backbone is production model; vLLM integration for high throughput; Qdrant with multi-vector for storage at 3MB/page; hybrid retrieval text + ColPali + RRF fusion; case studies: legal docs, Airbnb) + Multimodal Embedding Models (CLIP architecture: text-image contrastive learning; modern models: OpenAI CLIP ViT-L/14 / OpenCLIP ViT-bigG / BGE-M3 / Jina CLIP v2 89-langs / SigLIP; Qdrant named vectors text+image; LlamaIndex multimodal RAG; StayBot image-based listing search; MARS multimodal research agent; benchmark MTEB-R vs MTEB-I) + Video and Audio RAG (Whisper large-v3-turbo / faster-whisper 4× faster; pyannote-audio for speaker diarization; audio RAG pipeline; CLAP for audio-without-text search; video frame extraction + scene detection with PySceneDetect; video RAG with CLIP frames + transcript chunks; GPT-4o vision for live streams; Gemini 1.5 native video understanding; customer support voice bot; StayBot voice search; TTS for voice responses) + Capstone (legal document multimodal RAG end-to-end: MultimodalPDFIngestor with Unstructured.io + ColQwen2 + TableTransformer; TextEmbedder con BGE-M3 multilingual; ImageEmbedder con CLIP; MultimodalVectorStore con Qdrant named vectors (text 1024-dim BGE-M3 + image 768-dim CLIP + colpali 128-dim multi-vector); hybrid retrieval con RRF fusion; FastAPI service con GPT-4o vision; docker-compose con Qdrant+LangFuse+Postgres+API; benchmark text vs multimodal accuracy; 13th portfolio project: multimodal RAG skill). Cierra el gap crítico: multimodal RAG tenía solo menciones (06/16/05 Vision/Audio, 06/24 LlamaIndex multimodal) pero faltaba el pipeline completo end-to-end. Main thread, 6 sequential Writes, 0 subagents. Cross-links: 06/12 Production RAG, 06/13 vLLM, 06/16/05 Vision/Audio, 06/22 Instructor, 06/24 Haystack+txtai, 09/36 LangFuse, 10/33 Vector Databases. **Variance note:** 3,287 líneas vs 2,500 target (+31%). Notas 01 (4 PDF tools + OCR + tables + scanned detection), 02 (ColPali architecture + late interaction + vLLM + Qdrant multi-vector + benchmark), 03 (4 modern CLIP models + Qdrant named vectors + LlamaIndex multimodal + StayBot + MARS), 04 (Whisper + faster-whisper + pyannote + CLAP + scene detection + GPT-4o vision + Gemini native video), 05 (full legal documents capstone con 4 ingestors + 2 embedders + Qdrant multi-vector + RRF + GPT-4o vision) sobre target por la amplitud de modalidades y el capstone full-stack. | `06 - Large Language Models/26 - Multimodal Production RAG/` (6 files) |
| 46 | **Crear curso Cost Engineering as Discipline / FinOps for ML (09/41) — Tier-2 #6** | 6 notas, 2,821 líneas, English. Welcome (highest-paid discipline en 2026; cost crisis: $47K en 4 horas con runaway agent; 4 pillars of FinOps: visibility/allocation/optimization/forecasting; 14th portfolio project) + LLM Cost Fundamentals (token economics math: input vs output 3-5x ratio; cached vs fresh pricing; provider comparison GPT-4o/Claude/Llama 3/DeepSeek-V3; hidden costs: prefill, function calls, tool use rounds, reasoning tokens o1/o3, structured output, retries, failures; TCO calculator con input/output/cache/retry/tool/reasoning multipliers) + Cost Visibility (per-tenant attribution via LangFuse metadata; Prometheus counters with tenant_id/feature/model labels; per-tenant Grafana dashboards; showback vs chargeback; cost anomaly detection con z-score > 3; approval workflows para high-cost operations $50+; real case SaaS chargeback) + Cost Optimization Patterns (8 patterns: model tiering, prompt optimization, semantic cache, prefix cache Anthropic 90% off, batching, cost-aware routing LiteLLM, quantization, reserved capacity; combined optimization: 60-90% reductions; tier-by-task routing; cache invalidation strategies; PII concerns) + Forecasting and Budget Management (Prophet time series with 95% confidence intervals; per-tenant forecasting; budget alerts: 80% warning, 100% critical; predictive alerts via predict_linear; per-tenant budget enforcement at gateway; approval workflow with 5-min timeout default deny; reserved capacity planning AWS Bedrock/Azure 30-50% discount; cost-aware deployment gates in CI; executive CFO reports with optimization recommendations) + Capstone (FinOps service multi-tenant LLM end-to-end: CostAttribution via LangFuse metadata + Prometheus counters; CostForecaster con Prophet; BudgetEnforcer en gateway; ApprovalWorkflow con Slack integration y 5-min timeout; CostOptimizationRecommender con 4 recommendations types; FastAPI service con /forecast /budget/check /approval/request /approval/{id}/approve /recommendations /report; docker-compose con Redis + Prometheus + LangFuse + Grafana; 14th portfolio project: cost engineering skill). Cierra el gap crítico: FinOps para ML tenía notas dispersas (06/19 Gateway, 06/23 cost optimization) pero faltaba el practice discipline completo: visibility+allocation+optimization+forecasting. Main thread, 6 sequential Writes, 0 subagents. Cross-links: 06/19 LLM Gateway, 06/22 Instructor, 06/23 Serverless, 09/36 LangFuse, 09/39 Incident Response, 10/22 Cloud Computing, 10/33 Vector Databases. **Variance note:** 2,821 líneas vs 2,200 target (+28%). Notas 01 (TCO calculator con 6 multiplicadores + 4-provider comparison + 7 hidden costs), 02 (per-tenant attribution con metadata + Grafana + showback/chargeback + z-score anomaly), 03 (8 optimization patterns + 4 case studies + combined optimization table 85% savings), 04 (Prophet + budget alerts + approval workflow + reserved capacity + exec reports), 05 (full FinOps service capstone con 5 services + dashboards) sobre target por la profundidad de las 4 pillars de FinOps. | `09 - MLOps y Produccion/41 - Cost Engineering as Discipline - FinOps for ML/` (6 files) |
| 42 | **Reforzar cobertura async en el vault (B+C+D)** | 8 notas, 4,225 líneas, English/Spanish mix. B = Nueva nota 11 en `10/31 - FastAPI for ML` (560 líneas): "Advanced Async Patterns — Cancellation, Debugging, and Testing". Cubre `asyncio.TaskGroup` (Python 3.11+) structured concurrency, `asyncio.CancelledError` propagation, `asyncio.shield` patterns, timeouts (always-required pattern), bounded concurrency con `asyncio.Semaphore`, async generators + streaming, `pytest-asyncio` con `AsyncMock` + `ASGITransport`, debugging de async hangs con py-spy + faulthandler, 7 async pitfalls. C = Nueva nota 06 en `10/40 - Background Jobs and Workers` (620 líneas): "Async Debugging in Production — Common Pitfalls and Detection". Cubre los 7 failure modes en producción (zombie tasks, event loop starvation, semaphore deadlock, lost task references, N+1 awaits, unbounded fan-out, async gen misuse), async health endpoint (`/health/async`), py-spy playbook, faulthandler with SIGUSR1, custom watchdog, async test patterns including router ASYNC linter en CI, async runbook. D = Nuevo curso `03/08 - Async Python Patterns Reference` (6 notas, 3,045 líneas): 00 Welcome (ecosystem overview + when async is wrong tool) + 01 Event Loop Internals (uvloop, selector, Windows IOCP, GIL interplay, asyncio.Runner Python 3.12+, TCP keepalive) + 02 Library Ecosystem Decision Framework (HTTP: httpx vs aiohttp; DB: asyncpg vs psycopg 3; Redis; jobs: ARQ vs Celery vs Dramatiq; queues: aio-pika, aiokafka; validation: Pydantic vs attrs/cattrs; locks: redis SET NX EX, pottery.Redlock) + 03 Anti-Patterns Reference Card (20 patterns con stack-trace signatures: sync I/O in async, missing await, cancelled error, unbounded gather, no timeout, CPU bound, loop in loop, lost task, async gen misuse, N+1 awaits, semaphore deadlock, sync subprocess, cross-thread loop, race condition, unbounded queue, sync DNS, requests in async, sync sleep in tests, sync lib in async, test fixture leak) + 04 Subinterpreters (Python 3.12+ PEP 684 per-interpreter GIL, `InterpreterPoolExecutor`, hybrid async + subinterpreter pattern, decision tree vs multiprocessing vs to_thread) + 05 Capstone (build `asyncrunner` library: `run_parallel` + `retry` + `trace` + `shield` + `gather_with_limit` + `first_completed` + anti-pattern regression tests). Main thread, sequential Writes, 0 subagents. **Por qué:** usuario pidió reforzar async coverage antes de Plan T. Vault ya tenía buena cobertura (FastAPI 920 líneas, SQLAlchemy 7 notas, Background Workers 5 notas, Qdrant async patterns) pero le faltaban: un reference card de 20 antipatterns, subinterpreter deep dive, runbook de async debugging en producción. Cross-links: 03/03/06 Concurrencia, 10/31 FastAPI, 10/38 SQLAlchemy Async, 10/40 Background Workers, 10/33 Qdrant Production Async, 09/39 Incident Response.

| 47 | **Crear curso Portkey AI Gateway and Observability (06/27) — Plan X** | 6 notas, 2,369 líneas, English. Welcome (Portkey vs LiteLLM comparison table 13 features; 4 killer features) + Portkey Core Gateway Fundamentals (setup, providers 250+, request transforms, prompt templates, caching semantic + exact) + Observability and Cost Tracking (per-tenant dashboards, metadata propagation, custom properties, webhooks, budget alerts, Langfuse export) + Fallbacks and Conditional Routing (load balancing weighted, retry with exponential backoff, conditional routing by request body, canary deployments) + PII Redaction and Compliance (built-in PII types email/phone/SSN/credit-card, custom regex, GDPR HIPAA SOC2 audit logs, Vault integration) + Capstone (16th portfolio project: Portkey gateway routing OpenAI/Anthropic/Together + per-tenant cost + PII redaction + fallbacks + audit + Docker Compose). Cierra el gap de "enterprise LLM gateway with built-in observability + compliance". Cross-links: 06/19 LiteLLM, 09/36 LangFuse, 06/22 Instructor, 06/23 Serverless, 06/25 AI Compliance, 09/41 Cost Engineering. **Variance note:** 2,369 líneas vs 2,400 target (-1%). Notas 04 (PII/compliance/audit) y 05 (Capstone full Docker Compose) sobre target por la profundidad operacional. | `06 - Large Language Models/27 - Portkey AI Gateway and Observability/` (6 files) |
| 48 | **Crear curso BentoML Production Model Serving (09/42) — Plan Y** | 6 notas, 2,785 líneas, English. Welcome (multi-framework multi-target table; BentoML as standard for production serving) + Fundamentals (Bento artifact, `bentoml.sklearn/pytorch/xgboost.save_model`, signatures batchable, `bentoml.Service` decorator, `service-api` for I/O schemas) + Service Patterns (REST/gRPC sync/async, adaptive batching with `bentoml.api(batchable=True)`, dynamic batching, custom runners with `@bentoml.Runnable` for GPU models, monitoring with Prometheus metrics) + Distributed Serving (multi-model services, model ensembles, GPU sharing with CUDA_VISIBLE_DEVICES, model composition pipeline, custom inference graphs) + Deployment Targets (Docker via `bentoml build` + `bentoml containerize`, Kubernetes with `bentoctl` and BentoDeployment CRD, AWS Lambda via custom container, SageMaker endpoint, GCP Cloud Run, Azure ML) + Capstone (production BentoML platform: multi-model ensemble serving + adaptive batching + GPU sharing + K8s deployment with autoscaling + monitoring + CI/CD with GitHub Actions + Docker Compose stack). Cierra el gap crítico: vault tenía 0 dedicated notes para BentoML (mencionado solo en comparativas con TorchServe/KServe). Cross-links: 09/30 TorchServe, 09/32 KServe, 10/22 Cloud Computing, 02 Docker, 04/04 K8s, 04/03 Ray. **Variance note:** 2,785 líneas vs 2,500 target (+11%). Notas 03 (runners + batching) y 05 (Capstone con K8s + autoscaling) sobre target por la integración operacional. | `09 - MLOps y Produccion/42 - BentoML Production Model Serving/` (6 files) |
| 49 | **Crear curso TypeScript for AI Engineers (17/01) — Plan Z** | 6 notas, 2,844 líneas, English. Welcome (3-layer AI stack 2026: TS frontend/edge + Python core; ecosystem: Vercel AI SDK / LangChain.js / Cloudflare Workers AI / Bun / tRPC) + TypeScript Fundamentals for Python Developers (type system strict mode, generics + conditional types, async/await + Promise.allSettled, modules + ESM, discriminated unions, type inference vs annotation) + Next.js + Vercel AI SDK (App Router, Server Components, `useChat` hook, streaming with `streamText`, RSC for AI, `generateText` + tool calling, edge runtime deployment) + Cloudflare Workers for Edge AI (Workers AI catalog Llama 3.3 / Mistral / Qwen, Durable Objects for stateful AI, Vectorize for embeddings, KV for caching, sub-50ms global latency, `<50ms p99` with Smart Placement) + LangChain.js + AI SDK Integration (`ChatOpenAI` + `ChatAnthropic`, structured output with Zod, agent executor, OpenAI SDK with type-safe tools, tRPC for end-to-end types) + Capstone (full-stack TypeScript AI app: Next.js 15 chat UI with streaming + Cloudflare Workers inference backend + LangChain.js orchestration + tRPC for types + Vectorize for RAG + Durable Objects for session state + Vercel + Cloudflare deployment). Cierra el gap crítico: vault tenía 0 dedicated notes para TypeScript (mencionado solo en passing). Cross-links: 06/19 LLM Gateway, 06/12 Production RAG, 06/22 Instructor, 10/31 FastAPI, 13/06 Go ML Backend. **Variance note:** 2,844 líneas vs 2,400 target (+18%). Notas 02 (TS fundamentals completos), 03 (Next.js + Vercel AI SDK con RSC), 04 (Cloudflare Workers AI con Durable Objects), 05 (LangChain.js + AI SDK) todas sobre target por la amplitud de los runtimes modernos. | `17 - TypeScript Engineering/01 - TypeScript for AI Engineers/` (6 files) |
