...
Build a globally distributed sentiment analysis API using Fermyon Spin and WASI. The service deploys to 300+ edge locations, processes text with a pre-trained ML model, and returns predictions in under 10ms p99 latency.

### Functional Requirements

1. Deploy to 30+ edge locations globally
2. Process 10,000+ requests per second per region
3. Cold start under 1ms for each invocation
4. Store and load models from edge key-value storage
5. Support A/B testing with multiple model versions
6. Provide real-time metrics and health monitoring

### Main Components

- **Spin HTTP Handler**: Request routing and response formatting
- **ML Inference Component**: Candle-wasm model execution
- **KV Store Integration**: Model versioning and caching
- **Health Monitor**: Liveness and readiness probes
- **Metrics Exporter**: Prometheus-compatible metrics

### Success Metrics

- Global p99 latency: <10ms
- Cold start: <1ms
- Model size: <10MB compressed
- Availability: 99.99%
- Cost: <$100/month at 1M requests/day
